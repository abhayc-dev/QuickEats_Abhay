    ...
    <div className="bg-white rounded-xl shadow-lg p-5 space-y-5 border border-gray-200">
      ...
      {data?.shopOrders?.map((shopOrder, index) => (
        ...
        >
                  ...
                  {shopOrder.status === "delivered" && (
                    <div className="flex space-x-1 mt-2">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <button
                          key={star}
                          className={`text-lg cursor-pointer transition ${(selectedRating[itemId] || 0) >= star
          ...
          {/* //! Subtotal & Actions */}
          <div className="flex justify-between items-center border-t pt-2 border-gray-200 mt-2">
            ...
            <div className="flex items-center gap-2">
              {shopOrder.status === "delivered" && (
                <button
                  className="text-xs px-3 py-1 bg-yellow-100 text-yellow-700 font-semibold rounded-full hover:bg-yellow-200 transition"
                  onClick={() => {
              ...
              <span className={`text-sm font-medium capitalize ${shopOrder.status === 'delivered' ? 'text-green-600' : 'text-blue-600'
                }`}>
                {shopOrder.status}
              </span>
            </div>
