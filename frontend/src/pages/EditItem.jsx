//! This is a Edit Item Page By Owner 

import React, { useEffect, useState } from "react";
import { useDispatch } from "react-redux";
import { useNavigate, useParams } from "react-router-dom";
import { ArrowLeft, Utensils, UploadCloud, IndianRupee, ArrowRight, LayoutList, Leaf } from "lucide-react";
...
const EditItem = () => {
  ...
  const { itemId } = useParams(); //you get item id
  ...
  useEffect(() => {
    const handleGetItemById = async () => {
      try {
        ...
        setCurrentItem(result?.data); // Only after result exists
      } catch (error) {
  ...
  //! Sync state when currentItem changes
  useEffect(() => {
    if (currentItem) {
      ...
      <div className="w-full max-w-lg bg-white shadow-2xl rounded-[2rem] p-8 sm:p-10 border border-gray-100 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex flex-col items-center mb-8 text-center">
            <div className="w-16 h-16 bg-gradient-to-br from-orange-400 to-red-500 rounded-2xl flex items-center justify-center text-white mb-4 shadow-lg shadow-orange-200 -rotate-3">
              <Utensils size={32} />
            </div>
            ...
            <p className="text-gray-500 mt-2">
              Update your delicious dish details.
            </p>
          </div>
